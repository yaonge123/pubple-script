...
var SUBTITLE_FONTSIZE_MIN = -5; // 최소 단계 (기본값 + (SUBTITLE_FONTSIZE_MIN * SUBTITLE_FONTSIZE_MODIFY))
var SUBTITLE_FONTSIZE_MAX = 5; // 최대 단계
...
var VIDEO_INOUT_MARGIN_COVER = 250; // 커버 인아웃 체크 여유 공간(상하좌우 전부 적용)
...
var mediaplayer_customizer = function (p_parent) {
        ...
        return {
                        ...
                        for (var ii = 0 ; ii < ar.length ; ii++) {
                                ...
                                switch (ar_detail[0]) {
                                        ...
                                        case "btnonly":
                                                ...
                                                break;
                                        case "scroll": option.subtitle_scroll = true; break; // 자막에 스크롤 있음
                                        case "fullscreentarget":
                                                option.exist_fullscreen = true;
                                                option.fullscreen_target = true; // 풀스크린을 위한 타겟 미디어플레이어
                                                break;
                                        case "minimum":
                                        ...
                                        case "music":
                                                ...
                                                option.exist_barTime = true; // 시간막대
                                                option.exist_laeTime = true; // 시간레이블
                                                option.exist_notes = true; // 악보
                                                option.exist_btnListen = true; // 전체듣기/소절듣기 버튼 (악보 자동 설정)
                                                option.exist_btnWayloop = true; // 웨이포인트 전용 루프 버튼 (악보 자동 설정)
                                                option.exist_btnFocus = true; // 포커스 가시여부 조절 버튼 (악보 자동 설정)
                                                option.exist_btnLyrics = true; // 가사 가시여부 조절 버튼 (악보 자동 설정)
                                                option.exist_btnTune = true; // 튠 조절 버튼 (악보 자동 설정)
                                                option.exist_playbackrate = true; // 재생속도 조절
                                                break;
                                        case "full_video":
                                                option.exist_btnPlay = true; // 재생버튼
                                                option.exist_btnStop = true; // 정지버튼
                                        ...
                                        case "btnListen": option.exist_btnListen = true; break; // 전체듣기/소절듣기 버튼 (악보 자동 설정)
                                        case "btnWayloop": option.exist_btnWayloop = true; break; // 웨이포인트 전용 루프 버튼
                                        case "btnFocus": option.exist_btnFocus = true; break; // 포커스 가시여부 조절 버튼
                                        case "btnLyrics": option.exist_btnLyrics = true; break; // 가사 가시여부 조절 버튼
                ...
                btnPlay_onInit: function () {
                        ...
                        hie.btnPlay = tools_addElement("<div class='btnProto btnPlay'></div>", hie.controlPannel);
                },
                ...
                btnStop_onInit: function () {
                        ...
                        if (hie.mediaControls == null) { hie.mediaControls = tools_addElement("<div class='" + cat + "Controls'></div>", hie.mediaWrap); }
                        if (hie.controlPannel == null) { hie.controlPannel = tools_addElement("<div class='controlPannel'></div>", hie.mediaControls); }
                        hie.btnStop = tools_addElement("<div class='btnProto btnStopOff'></div>", hie.controlPannel);
                },
                ...
                volume_onInit: function () {
                        ...
                        // UI
                        if (hie.mediaControls == null) { hie.mediaControls = tools_addElement("<div class='" + cat + "Controls'></div>", hie.mediaWrap); }
                        hie.btnSound = tools_addElement("<div class='btnProto btnSoundOn'></div>", hie.mediaControls);
                ...
                playbackrate_onInit: function () {
                        ...
                        switch (OPTION_PLAYBACKRATE_TYPE) {
                                ...
                                case 1:
                                        ...
                                        break;
                                case 2:
                                        hie.btnPlaybackrate = tools_addElement("<div class='btnProto btnPlaybackrateNormal'></div>", hie.speedPannel);
                                        hie.menuPlaybackrate = tools_addElement("<div class='menuDropDown'></div>", hie.speedPannel);
                ...
                loop_onInit: function () {
                        ...
                        // UI
                        if (hie.mediaControls == null) { hie.mediaControls = tools_addElement("<div class='" + cat + "Controls'></div>", hie.mediaWrap); }
                        hie.btnLoop = tools_addElement("<div class='btnProto btnLoopOff'></div>", hie.mediaControls);
                },
                ...
                fullscreen_onInit: function () {
                        ...
                        // UI
                        if (hie.mediaControls == null) { hie.mediaControls = tools_addElement("<div class='" + cat + "Controls'></div>", hie.mediaWrap); }
                        hie.btnFullscreen = tools_addElement("<div class='btnProto btnFullscreenOff'></div>", hie.mediaControls);
                },
                ...
                notes_onInit: function () {
                        ...
                        if (option.exist_btnListen == true) {
                                hie.btnListen = tools_addElement("<div class='btnProto btnListenAll'></div>", hie.mediaControls);
                                hie.menuListen = tools_addElement("<div class='menuDropDown'></div>", hie.btnListen);
                                hie.btnListenAll = tools_addElement("<div class='btnProto btnListenAll'></div>", hie.menuListen);
                                hie.btnListenPart = tools_addElement("<div class='btnProto btnListenPart'></div>", hie.menuListen);
                                hie.btnWaypointPrev = tools_addElement("<div class='btnProto btnWaypointPrev'></div>", hie.mediaControls);
                                hie.btnWaypointLabel = tools_addElement("<div class='btnProto btnWaypointLabel'></div>", hie.mediaControls);
                                hie.btnWaypointLabel.label = tools_addElement("<div class='label'></div>", hie.btnWaypointLabel);
                                hie.btnWaypointNext = tools_addElement("<div class='btnProto btnWaypointNext'></div>", hie.mediaControls);
                                hie.menuWaypoint = tools_addElement("<div class='menuDropDown'></div>", hie.btnWaypointLabel);
                        }
                        if (option.exist_btnWayloop == true) { hie.btnWayloop = tools_addElement("<div class='btnProto btnWayloopOff'></div>", hie.mediaControls); }
                        if (option.exist_btnFocus == true) { hie.btnFocus = tools_addElement("<div class='btnProto btnFocusOff'></div>", hie.mediaControls); }
                        if (option.exist_btnLyrics == true) { hie.btnLyrics = tools_addElement("<div class='btnProto btnLyricsOff'></div>", hie.mediaControls); }
                        if (option.exist_btnTune == true) {
                                hie.btnTune = tools_addElement("<div class='btnProto btnTuneNormal'></div>", hie.mediaControls);
                                hie.menuTune = tools_addElement("<div class='menuDropDown'></div>", hie.btnTune);
                                hie.btnTuneUp = tools_addElement("<div class='btnProto btnTuneUp'></div>", hie.menuTune);
                                hie.btnTuneNormal = tools_addElement("<div class='btnProto btnTuneNormal'></div>", hie.menuTune);
                                hie.btnTuneDown = tools_addElement("<div class='btnProto btnTuneDown'></div>", hie.menuTune);
                        }
                },
