...
function essayQuiz_correctBtn(){
            ...
            for(var j=0, jLen=quiz.length; j<jLen; j++){
                ...
                if(quizId===quizScope && quizType==="essayQuiz"){
                    ...
                    if (global_solve) {
                        ...
                        essayQuiz(quizTmp, quizScope, totalQuiz, replayBtn);
                    } else {
                        // 정답 확인 실행
    ...
    for(var i=0, iLen=eachBtn.length; i<iLen; i++) {
        eachBtn[i].addEventListener("click", function() {
            ...
            var btnAudio = parentsEle.getElementsByClassName("inner_mp3_wrap")[0];
            var btnAudioClass = btnAudio ? btnAudio.classList : null;
            var isToggle = btnAudio ? btnAudio.classList.contains("toggle") : "";
            ...
            if (quizType === "essayQuiz") {
                ...
                if (preEle.classList.contains("essayAnswer")){
                    ...
                    if (!beforePreEle || !beforePreEle.classList.contains("input_wrap") && !beforePreEle.classList.contains("blank_box")) {
                        ...
                        do {
                            if (!beforePreEle.classList.contains("blank_box")) {
                        ...
                        } while (beforePreEle.classList.contains("input_wrap") || beforePreEle.classList.contains("blank_box"))
                ...
                if(!targetEls) {
                    ...
                    if (targetEle.previousElementSibling.disabled) {
                        targetEle.previousElementSibling.disabled = false;
                    } else {
                        targetEle.previousElementSibling.disabled = true;
                    }
                    targetEle.previousElementSibling.value = "";
                } else {
                    for (l = 0; l < answerCnt; l++) {
                        targetEls[l].classList.toggle("show");

                        if (targetEls[l].previousElementSibling.disabled) {
                            targetEls[l].previousElementSibling.disabled = false;
                        } else {
                            targetEls[l].previousElementSibling.disabled = true;
                        }
                        targetEls[l].previousElementSibling.value = "";
                    }
                }
                ...
                // 오디오 버튼 토글
                if (btnAudio && isToggle) {
                    if (replayYn) {
                        btnAudioClass.remove("blind");
                    } else if (!btnAudioClass.contains("blind")) {
                        btnAudioClass.add("blind");
                    }
                }
...
function essayQuiz(quizEle, quizScope, totalQuiz, replayBtn) {
    ...
    // for slide contents
    var slideCont = quizEle.getElementsByClassName("slideContent");
    var slideLen = slideCont.length;
    var onSlide, i;

    // 듣기 버튼 활성화
    var btnAudio = quizEle.getElementsByClassName("inner_mp3_wrap")[0];
    var btnAudioClass = btnAudio ? btnAudio.classList : null;
    var isToggle = btnAudio ? btnAudio.classList.contains("toggle") : "";

    // 개별 버튼
    var btnEach = quizEle.getElementsByClassName("eachBtn");
    var btnEachLen = btnEach.length;

    // if (slideLen) {
    //     // quizCheckYn 변경
    //     for (i = 0; i < slideLen; i++) {
    //         onSlide = slideCont[i].querySelector(".detailList.on");

    //         if (replayBtn) {
    //             quizCheckYn = true;
    //         } else if (!onSlide.querySelector(".essayAnswer.show")) {
    //             quizCheckYn = false;
    //         } else {
    //             quizCheckYn = true;
    //         }
    ...
    if (quizCheckYn) {
        ...
        if (slideLen) {
            for (i = 0; i < slideLen; i++) {
                onSlide = slideCont[i].querySelector(".detailList.on");
                onSlide.getElementsByClassName("essayAnswer")[0].classList.remove("show");
                onSlide.getElementsByClassName("essayWrite")[0].disabled = false;
                
                if (replayBtn) {
                    onSlide.getElementsByClassName("essayWrite")[0].value = "";
                }
            }
        } else {
        ...
        // 개별 버튼 연동
        for (i = 0; i < btnEachLen; i++) {
            btnEach[i].classList.remove("on");
        }
    } else {
        // 정답 항목 표시
        quizInpCorrect = totalScope.getElementsByClassName("essayAnswer");
        ...
        if (slideLen) {
            for (i = 0; i < slideLen; i++) {
                onSlide = slideCont[i].querySelector(".detailList.on");
                onSlide.getElementsByClassName("essayAnswer")[0].classList.add("show");
                onSlide.getElementsByClassName("essayWrite")[0].disabled = true;
            }
        } else {
