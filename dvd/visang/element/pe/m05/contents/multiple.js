...
function choiceQuiz_init(){
	...
	var quizPart, checkItem, checkNum, correctCheck;
		...
		for(var i=0, iLen=quiz.length; i<iLen; i++){
			...
			if(quizType==="choiceQuiz"){
				...
				for(var j=0, jLen=quizPart.length; j<jLen; j++){
					checkNum = quizPart[j].getElementsByClassName("check_num");

					for(var k=0, kLen=checkNum.length; k<kLen; k++){
						checkNum[k].addEventListener("click", function(){
							var multi, multiLi;
							var ele, checkItemAll, radio, checkSelectAll;
							var parentsEle = this.closest(".quizType");
							var checkItem = this.parentElement;
							var answer = parentsEle.getElementsByClassName("answerCorrect")[0].textContent;		
							
							// 정답 보기 이후 동작 안함
							if (parentsEle.classList.contains("correct")) {
								return;
							}
							...
							if(multiYn){
								...
								if(!parentsEle.classList.contains("quizChk")){
									multi = checkItem.getElementsByClassName("checkSelect")[0];
									if(multi.checked){
										checkItem.classList.remove("on");
										multi.checked = false;
								...
								if(!parentsEle.classList.contains("quizChk")){
									...
									radio = checkItem.getElementsByClassName("checkSelect")[0];

									// input check 처리
									if(radio.checked){
										radio.checked = false;

										// checkItem.classList.remove("right");
										// checkItem.classList.remove("wrong");
										...
										if (radio.value === answer) {
											correctCheck++;
											multiLi = document.getElementsByClassName("type_multiple");
											//console.log('정답입니다.', );
											for(var x = 0; x < multiLi.length; x++){
												if (multiLi[x].classList.contains("on")){
													multiLi[x].classList.add("ok");
												}
											}
											
											multiLi = document.getElementsByClassName("type_multiple ok");
											if (correctCheck >= 3 && multiLi.length === 3) {
												gold_Animate("allright");
												$(".type_all_right").show();
											}

											checkItem.classList.add("answer");
											gold_Animate("right");
											$(".type_right").show();
											setTimeout(function(){ 
												$(".btnNext").click(); 
...
function choiceQuiz_correctBtn(){
	...
	for(var i=0, iLen=quizBtn.length; i<iLen; i++){
		quizBtn[i].addEventListener("click", function(){
			...
			var totalQuiz = false;
			var isChk = this.classList.contains("btnCheck");
			...
			for(var j=0, jLen=quiz.length; j<jLen; j++){
				...
				if(quizId===quizScope && quizType==="choiceQuiz"){
					...
					if(global_solve){
						// 정답 확인 실행
						choiceQuiz(quizTmp, quizScope, totalQuiz, isChk);
					}else{
						// 정답 확인 실행
...
function choiceQuiz(quizEle, quizScope, totalQuiz, isChk){
	...
	if(!isChk){
		...
		for(var k=0, kLen=quizPart.length; k<kLen; k++){
			...
			for(var x=0, xLen=checkItemAll.length; x<xLen; x++){
				...
				checkItemAllTmp.classList.remove("wrong");
			}
		...
		if(multiYn){
			...
			for(var k=0, kLen=quizPart.length; k<kLen; k++){
				...
				for(var y=0, yLen=quizChkSel.length; y<yLen; y++){
					quizChkSelTmp = quizChkSel[y];
					quizChkSelTmp.parentNode.classList.remove("wrong");
