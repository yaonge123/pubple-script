...
PUBPLE.ui = (function() {
    ...
    var initBtnShow = function(el) {
        ...
        // 정답 보기 버튼 초기화 - 181121
        el.querySelector(".btnAllShow").style.display = "inline-block";
        el.querySelector(".btnAgain").style.display = "none";
    }
    ...
    return {
        ...
        clickBtnShow: function() {
            ...
            var againBtns = document.getElementsByClassName("btnAgain");
            var againBtnLen = againBtns.length;
            var i, j, k;
            var btn;
            ...
            for (i = 0; i < btnLen; i++) {
                ...
                btn.addEventListener("click", function() {
                    ...
                    // 정답 보기 버튼 연동 - 181121
                    var quizBox = this.closest(".quiz_box");
                    if (!quizBox) {
                        quizBox = this.closest(".ans_box");
                        if (!quizBox) return;
                    }
                    var quizBoxId = quizBox.id;
                    var hideBoxes = quizBox.querySelectorAll(".hideTxtBox");
                    var hideBoxLen = hideBoxes.length;
                    var btnAllShow = document.querySelector(".btnAllShow[data-target='" + quizBoxId + "']");
                    var btnAgain = btnAllShow.nextElementSibling;
                    var onTxtLen = 0;
                    var hideBox, m;

                    for (m = 0; m < hideBoxLen; m++) {
                        hideBox = hideBoxes[m];
                        if (hideBox.classList.contains("blind")) {
                            onTxtLen++;
                        }
                        if (onTxtLen === hideBoxLen) {
                            btnAllShow.style.display = "none";
                            btnAllShow.nextElementSibling.style.display = "inline-block";
                        } else {
                            btnAllShow.style.display = "inline-block";
                            btnAllShow.nextElementSibling.style.display = "none";
                        }
                    }
                ...
                for (j = 0; j < answerBtnLen; j++) {
                    answerBtns[j].addEventListener("click", function() {
                        ...
                        for (; i < targetBtnLen; i++) {
                            ...
                            if (!this.classList.contains("btnAgain")) {
            ...
            // 다시 풀기 버튼 있는 경우
            if (againBtnLen) {
                for (k = 0; k < againBtnLen; k++) {
                    againBtns[k].addEventListener("click", function() {
                        var targetId = this.previousElementSibling.getAttribute("data-target");
                        var targetArea = document.getElementById(targetId);
                        var hideBoxes = targetArea.getElementsByClassName("hideTxtBox");
                        var hideBoxLen = hideBoxes.length;
                        var i;
        ...
        initTab: function(titleChange) {
            ...
            var  titleChange_fn = null;

            if(titleChange){
                titleChange_fn = function(index){
                    var titleLayers = document.querySelectorAll('.content .titleWrap .title');
                    for( var k = 0; titleLayers.length > k; k++ ){
                        titleLayers[k].style.display = 'none';
                    }
                    titleLayers[index].style.display = 'block';
                }
            }
        ...
        setBtnAudio: function(url){
            ...
            $(".content").find(".popUp, .tabMenu li, .btnPrev, .btnNext").each(function(){
                $(this).click(function(e){
                    if($(this).hasClass('off')) return;
                    if (typeof e.originalEvent !== "undefined") {
                        playSound();
                    } else if ($(".type_right").css("display") === "none"){
                        playSound();
                    }
                });
            });
            ...
            if(!tabLen) {
                ...
                for (; i < tabLen; i++) {
                    tabs[i].addEventListener("click", function(e) {
                        ...
                        var tabsArr = [].slice.call(tabs);
                        var tabIdx = tabsArr.indexOf(target);
                        var thisTabIndex = this.getAttribute('data-tapindex');

                        currTabEl.classList.remove("on");
                        target.classList.add("on");
                        currDetail.classList.remove("on");

                        for(var index = 0; tabDetails.length > index; index++){
                            if(thisTabIndex == tabDetails[index].getAttribute('data-slideindex')){
                                tabDetails[index].classList.add('on');
                            }
                        }
        ...
        initTap_slide : function (){
            ...
            for (; i < slideContentsLen; i++) {
                ...
                moveBtnWrap.addEventListener("click", function(e) {
                    ...
                    if(nextPageEl.getAttribute('data-slideindex')){
                        var tabs = document.querySelectorAll(".tabMenu > li");
                        for(var index = 0; tabs.length > index; index++){
                            tabs[index].classList.remove('on');
                            if(tabs[index].getAttribute('data-tapindex') == nextPageEl.getAttribute('data-slideindex')){
         ...
         * 탭 연동 depth 구조 슬라이드
         * @memberOf PUBPLE.ui
         * @param defaultLayerFlag (boolean) //인트로 화면 유무
         * @param innerSlideCtrl (boolean)  //슬라이더 in 슬라이더 콘트롤 유무
         */
        multiSlider : function(_defaultLayer){


            var depth_1_slider = '';
            var depth_2_slider = '';
            //슬라이드 wrapper
            var slideContEl = document.querySelector('.slideContent');
            //슬라이드 클래스 str
            var slide_class = '_multiSlider';
            //슬라이드 콘텐츠 클래스 str
            var slide_content_class = '_slideDetail';
            //모든 슬라이드 get
            var allSlide_nodes = document.querySelectorAll('.' + slide_class);
            //슬라이드 콘텐츠 get
            var slider_contents = allSlide_nodes[0].querySelectorAll('.' + slide_content_class);
            //현재 보고 있는 콘텐츠 index
            var view_slide_index = 1;
            //전체 콘텐츠 length
            var slide_content_len = slider_contents.length;
            //텝 버튼 등록
            var tabBtns = document.querySelectorAll('.tabMenu [data-multi_tap_index]');

            //슬라이드 이동 버튼
            var moveBtnWrap = util.createEl("div", slideContEl, "beforeend");
            var prevEl = util.createEl("div", moveBtnWrap, "beforeend", "btnPrev");
            var nextEl = util.createEl("div", moveBtnWrap, "beforeend", "btnNext");
            ...
            var self_func = {};
            //set defaultLayer
            self_func.defaultNode;
            //슬라이드 depth val
            self_func.setDepthVal = 1;

            //초기값 셋
            self_func.init = function() {

                for( var k = 0; allSlide_nodes.length > k; k++ ){
                    self_func.checkParentDom(allSlide_nodes[k] , '_multiSlider' , true);
                }
                //이벤트 등록
                this.bindEvt();

                if(_defaultLayer){
                    this.showSlide('default');
                }else{
                    //최초 슬라이드 on
                    this.showSlide(1);
                }
            ...
            //부모 node 체크
            //재귀함수
            self_func.checkParentDom = function(dom , classStr , self_node){
                if(self_node) this.defaultNode = dom;
                if(dom.parentNode.tagName.toUpperCase() === 'DOCUMENT' || dom.parentNode.tagName.toUpperCase() === 'BODY'){
                    this.defaultNode.setAttribute('data-slider_depth' , 1);
                    self_func.setDepthVal = 1;
                    return false;
                }
                if(dom.parentNode.classList.contains(classStr)){
                    this.defaultNode.setAttribute('data-slider_depth' , 2);
                    self_func.setDepthVal = 2;
                    innerSlideLen ++;
                    return true;
                }
                this.checkParentDom(dom.parentNode , classStr);
            };

            //이벤트 등록
            self_func.bindEvt = function(){
                prevEl.addEventListener('click' , function(){
                    self_func.gotoSlide(-1);
                });
                nextEl.addEventListener('click' , function(){
                    self_func.gotoSlide(+1);
                });
                for( var  i = 0; tabBtns.length > i; i++ ){
                    tabBtns[i].addEventListener('click' , function(){

                        if(this.classList.contains('on')){
                            if(_defaultLayer){
                                this.classList.remove('on');
                                self_func.showSlide('default');
                            } else {
                                return;
                            }
                        } else {
                            for(var index = 0; tabBtns.length > index; index++){
                                tabBtns[index].classList.remove('on');
                            }
                            this.classList.add('on');
                            self_func.showSlide(this.getAttribute('data-multi_tap_index'));
                        }
                    });
                }
            };
            ...
            //부모 class 체크
            self_func.checkParentClass = function(dom ,  classStr){

                var tempCheckVal = true;
                ...
                if(dom.parentNode.classList.contains(classStr)){
                    ...
                    for( var  i = 0; tabBtns.length > i; i++ ){
                        ...
                        if(tabBtns[i].getAttribute('data-multi_tap_index') ==  dom.parentNode.getAttribute('data-multi_slide_index')){
                ...
                if(tempCheckVal){
                    this.checkParentClass(dom.parentNode , classStr);
                }
            };


            //slide num ctrl
            self_func.gotoSlide = function(mergeSlideNum) {
                var gotoSlideNum = view_slide_index + mergeSlideNum;
                if(gotoSlideNum < 2) gotoSlideNum = 1;
                if(gotoSlideNum >= slide_content_len) gotoSlideNum = slide_content_len;
                view_slide_index = gotoSlideNum;
                ...
                this.showSlide(gotoSlideNum , moveFlag);
            };
            //slide show
            self_func.showSlide = function(slideNum) {

                if(slideNum == 'default'){
                    _defaultLayer.style.display = 'block';
                    prevEl.style.display = 'none';
                    nextEl.style.display = 'none';
                    return
                }else{
                    _defaultLayer.style.display = 'none';
                }

                prevEl.style.display = 'block';
                nextEl.style.display = 'block';

                if(slideNum == 1) prevEl.style.display = 'none';
                if(slideNum >= slide_content_len - innerSlideLen) nextEl.style.display = 'none';

                //탭 온
                for( var  i = 0; tabBtns.length > i; i++ ){
                    tabBtns[i].classList.remove('on');
                    if(tabBtns[i].getAttribute('data-multi_tap_index') == slideNum){
                        tabBtns[i].classList.add('on');
                    }
                }

                //전체 슬라이드 콘텐츠 disply none
                for( var i = 0; slider_contents.length > i; i++ ){
                    slider_contents[i].style.display = 'none';
                    if(slider_contents[i].getAttribute('data-multi_slide_index') == slideNum){

                        slider_contents[i].style.display = 'block';

                        //슬라이드 콘텐츠 확인
                        var check_InnerLayer = slider_contents[i].querySelectorAll('.' + slide_class);
                        //inner 슬라이드 true
                        if(check_InnerLayer.length > 0) {
                            check_InnerLayer[0].querySelectorAll('.' + slide_content_class)[0].style.display = 'block';
                        } else {
                            self_func.checkParentClass(slider_contents[i] , slide_content_class);
                        }
                    }
                }

                view_slide_index = Number(slideNum);
            };

            try {
                self_func.init();
            } catch(e) {
                console.error('error fn : PUBPLE.ui.multiSlider , ' ,  'error msg : ' , e);
            }
