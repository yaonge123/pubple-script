...
PUBPLE.ui = (function() {
    ...
    return {
        ...
        clickBtnShow: function() {
            ...
            var pTagShowBtns = document.getElementsByClassName("btnpTagShow");
            var pTagShowLen = pTagShowBtns.length;
            ...
            var pBlind = document.getElementsByClassName("p_blind");
            var i, j, k;
            var btn;
            ...
            //p태그 투명에서 노란색으로 바꾸기
            if (pTagShowLen) {
                for (i = 0; i < pTagShowLen; i++) {
                    pTagShowBtns[i].addEventListener("click", function () {
                        for (var j = 0; j < pBlind.length; j++) {
                            if (!pBlind[j].style.opacity) {
                                pBlind[j].setAttribute("style", "opacity:1;");
                            } else if (pBlind[j].style.opacity === '0') {
                                pBlind[j].style.opacity = '1';
                            } else if (pBlind[j].style.opacity === '1') {
                                pBlind[j].style.opacity = '0';
                            }
                        }
                    });
                }
