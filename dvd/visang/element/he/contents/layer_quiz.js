...
function layerQuiz_init(){
	...
	var quizPartTmp;
	var quizItem, quizBox, quizItemTmp;
	var quiz, quizType, quizCorrect;
	...
	if(global_solve){
		...
		quizItem = document.getElementsByClassName("layerItem");
		for(var j=0, jLen=quizItem.length; j<jLen; j++){
			quizItem[j].addEventListener("click", function(){
				var content = this.getAttribute("data-content") || this.innerText;
				var quizPart = document.getElementsByClassName("layerWrite");
				var quizPartTmp;
				...
				for(var i=0, iLen=quizPart.length; i<iLen; i++){
					...
					if(quizPartTmp.classList.contains("sel")){
						...
						// 레이어 초기화
						layerQuiz_removeBox();
						quizItem[i].parentNode.classList.remove("show");
					}else{
						if(quizPartTmp.value===content){
							quizPartTmp.value = "";
						}
	...
	for(var i=0, iLen=quiz.length; i<iLen; i++){
		...
		if(quizType==="layerQuiz"){
			...
			quizCorrect = (quizPartTmp.getElementsByClassName("answerCorrect")[0].innerText).split("//");
			
			for(var j=0, jLen=quizItem.length; j<jLen; j++){
				quizItemTmp = quizItem[j];
				if(quizItemTmp.getAttribute("data-content")===null){
					quizPart[quizCorrect[j]-1].innerText = quizItemTmp.innerText;
				}
			}
		}
	}
...
function layerQuiz(quizEle, quizScope){
	var quizPartTmp;
	...
	var quizCheck = true;
	var quizCorrect = (quizEle.getElementsByClassName("answerCorrect")[0].innerText).split("//");
