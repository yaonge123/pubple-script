...
function choiceQuiz_init(){
	...
	if(global_solve){
		for(var i=0, iLen=quiz.length; i<iLen; i++){
			...
			correctSound = document.getElementsByClassName('feedAudio_ok')[0];
			wrongSound = document.getElementsByClassName('feedAudio')[0];
				...
				for(var j=0, jLen=quizPart.length; j<jLen; j++){
					...
					for(var k=0, kLen=checkItem.length; k<kLen; k++){
						checkItem[k].addEventListener("click", function(){
							var multi;
							var ele, checkItemAll, radio, checkSelectAll, checkWrongExpAll;
							
							var parentsEle = this;
							var checkAnswer = checkItem[strCorrect-1];
							var checkAnswerExp = checkAnswer.getElementsByClassName("checkExp");
							var checkWrongExp = this.getElementsByClassName("checkExp");
							...
							if(multiYn){
								...
								if(!parentsEle.classList.contains("quizChk")){
									...
									checkWrongExpAll = ele.getElementsByClassName("checkExp");

									for(var x=0, xLen=checkItemAll.length; x<xLen; x++){
										checkItemAll[x].classList.remove("on");
										checkItemAll[x].classList.remove("answer");
										if(checkAnswerExp && checkAnswerExp.length !== 0){
											checkAnswerExp[0].classList.remove("show");
										}else{
											for(var i = 0; i < checkWrongExpAll.length; i++){
												checkWrongExpAll[i].classList.remove("show");
											}
										}
										quizBtn[0].classList.remove("replay");
										quizBtn[0].classList.remove("checkBtn");
									}
									...
									if(radio.checked){
										...
										// 정답/오답 클릭할 경우
										if(checkAnswer.classList[1] == 'on'){
											quizBtn[0].classList.add("replay");
											quizBtn[0].classList.add("checkBtn");
											checkAnswer.classList.add("answer");

											if(checkAnswerExp.length !== 0){
												checkAnswerExp[0].classList.add("show");
											}
											...
											if(checkWrongExp.length !== 0){
												checkWrongExp[0].classList.add("show");
											}
...
function choiceQuiz_correctBtn(){
	...
	var onList = document.getElementsByClassName("checkItem on");
	var checkExp = document.getElementsByClassName("checkExp");
	...
	for(var i=0, iLen=quizBtn.length; i<iLen; i++){
		quizBtn[i].addEventListener("click", function(){
			...
			if(quizBtn[0].classList.contains("checkBtn")){
				for(var x = 0; x < onList.length; x++){
					onList[x].classList.remove("answer");
					onList[x].classList.remove("on");
					for(var y = 0; y < checkExp.length; y++){
						checkExp[y].classList.remove("show");
					}
				}
				quizBtn[0].classList.remove("checkBtn");
			}else{
...
function choiceQuiz(quizEle, quizScope, totalQuiz){
	...
	//	정답 보충설명
	var answerDesc = quizEle.getElementsByClassName("checkExp");
	var answerDescLen = answerDesc.length;
	var i = 0;
	...
	if(quizCheckYn){
		...
		// 정답 보충설명 표시
		for (; i < answerDescLen; i++) {
			answerDesc[i].classList.remove("show");
		}
		...
		// 정답 보충설명 표시
		for (; i < answerDescLen; i++) {
			answerDesc[i].classList.add("show");
		}
