...
function setupMediaPlayer() {
        ...
        if (typeof PUBPLE !== "undefined" && PUBPLE.ui !== "undefined" && PUBPLE.ui.scaleValue !== "undefined") {
                hq_scale_multiplier = 1 / PUBPLE.ui.scaleValue.x;
        }
...
// 인아웃 체크
function tools_chkInout(event, target) {
        // 준비
        var mx = event.pageX * hq_scale_multiplier;
        var my = event.pageY * hq_scale_multiplier;
        var tx0 = $(target).offset().left * hq_scale_multiplier + VIDEO_INOUT_MARGIN;
        var ty0 = $(target).offset().top * hq_scale_multiplier + VIDEO_INOUT_MARGIN;
        var tx1 = tx0 + $(target).width() - (VIDEO_INOUT_MARGIN * 2);
        var ty1 = ty0 + $(target).height() - (VIDEO_INOUT_MARGIN * 2);
