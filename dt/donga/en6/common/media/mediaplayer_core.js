// version 1.0.63 : 따라말하기 onEnded 보정
///// mediaplayer_core
var mediaplayer_core = function (p_parent) {
        ...
        var b_disable_input = false; // 인풋 금지(초기화 코드 없음. 일시적으로 사용하고 반드시 돌려 놓을 것)
        var i_media_state = 0; // 미디어 상태 (-2:로드실패, -1: 로딩중, 0:대기중, 1:로딩됨)
        
        // system
        var id_interval_actual_loaded; // 실제 로드 완료 체크 interval (duration 기준으로 체크)
        ...
        var i_anti_move_abnormality = -1; // ios 특수 예외 처리(currenttime 이 약간 앞으로 이동하는 현상 방어), castMove 로 지정된 시간값 저장.
        var id_anti_move_abnormality; // ios 특수 예외 처리 Timeout
        ...
        return {
                ...
                init: function () {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.init()"); }
                ...
                start: function (p_preload, p_resetonstop) {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.start(" + p_preload + ", " + p_resetonstop + ")"); }
                ...
                b_disable_input: function() { return b_disable_input; },
                ...
                onLoadedData: function () {
                        ...
                        if (dev_trace_detail == _parent._id()) { console.log("# core.onLoadedData()"); }

                        // 조건검사
                        if (b_loaded_media == true) { return; }

                        // 조건검사 : duration 기준으로 실제 로드 완료 체크
                        if (_parent._media().duration == 0 || isNaN(_parent._media().duration) == true) {
                                id_interval_actual_loaded = setInterval(function () {
                                        clearInterval(id_interval_actual_loaded);
                                        _this.onLoadedData();
                                }, 50);
                        ...
                        if (time_move_reserve != -1) {
                                _this.castMove(time_move_reserve, true);
                                time_move_reserve = -1;
                        }
                },
                onLoadFailure: function () {
                        ...
                        if (dev_trace_detail == _parent._id()) { console.log("# core.onLoadFailure()"); }
                ...
                onEnded: function () {
                        ...
                        if (dev_trace_detail == _parent._id()) { console.log("# core.onEnded()"); }
                        ...
                        // 정지판단
                        var is_stop = false;
                        if (_parent._category() == "video" && VIDEO_STOP_ON_END == true) { is_stop = true; }
                        if (_parent._category() == "audio" && AUDIO_STOP_ON_END == true) { is_stop = true; }
                        
                        // 예외처리
                        if (_parent._option().exception.onEndedStop == true) { is_stop = true; }
                        if (_parent._option().exception.onEndedPause == true) { is_stop = false; }

                        // 문장 클릭 재생 중 onEnded 무시
                        if (_parent._module().subtitle != null && _parent._module().subtitle.time_text_end() != -1) {
                                is_stop = false;
                                _parent._module().subtitle.releaseHighlight();
                        }

                        // 따라하기 진행 중 onEnded 처리
                        if (_parent._module().follow != null) { _parent._module().follow.onEnded(); }

                        // 동작
                        if (is_stop == true) { _this.castStop(); }
                        else { _this.castPause(); }

                        // RELEASE_FULLSCREEN_ON_ENDED
                        if (typeof (RELEASE_FULLSCREEN_ON_ENDED) != "undefined" && RELEASE_FULLSCREEN_ON_ENDED == true && _parent._module().fullscreen != null) { _parent._module().fullscreen.onEnded(); }

                        // 이벤트
                        if (typeof onMediaPlayerEvent == "function" && _parent._option().event.onEnded == true) { onMediaPlayerEvent("onEnded", _parent._id()); }
                },

                // 재생 갱신 이벤트
                onTimeUpdate: function () {
                        // 조건검사
                        if (i_anti_move_abnormality != -1 && parseInt(_parent._media().currentTime * 100) < i_anti_move_abnormality) { return; }
                        ...
                        if (mod.follow != null) { mod.follow.onTimeUpdate(); }
                        if (mod.roleplay != null) { mod.roleplay.onTimeUpdate(); }
                ...
                // 제어
                castPlay: function (option) {
                        // option
                        // 1:follow 의 재귀호출 방지

                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castPlay() try"); }
                        ...
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castPlay()"); }
                        ...
                        // 인터랙션
                        if (option != 1) { _this.onInteraction(); }
                ...
                castPause: function () {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castPause() try"); }

                        // 조건검사
                        if (b_loaded_media == false) { return; }

                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castPause()"); }
                ...
                castStop: function () {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castStop() try"); }
                        ...
                        if (_parent._media().paused == true && _parent._media().currentTime == 0) {
                                // 따라말하기 예외처리
                                if (_parent._module().follow != null) { _parent._module().follow.onCastStopInStop(); }
                                return;
                        }

                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castStop()"); }
                        ...
                        // 변수 초기화
                        time_move_reserve = -1;
                        i_anti_move_abnormality = -1;
                        if (id_anti_move_abnormality != null) {
                                clearTimeout(id_anti_move_abnormality);
                                id_anti_move_abnormality = null;
                        }
                ...
                castHardReset: function () {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castHardReset()"); }
                        ...
                        // 전달
                        if (mod.mpText != null) { mod.mpText.onReset(); }
                        ...
                        if (mod.btnChange != null) { mod.btnChange.onHardReset(); }
                        if (mod.btnAnotherChange != null) { mod.btnAnotherChange.onHardReset(); }
                        if (mod.follow != null) { mod.follow.onReset(); }
                ...
                castReset: function (onstop) {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castReset(" + onstop + ")"); }
                ...
                castMove: function (time, reserved) {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castMove(" + time + ") try"); }
                        ...
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castMove(" + time + ")"); }

                        // 인터랙션
                        if (reserved == null || reserved == false) { _this.onInteraction(); }
                        ...
                        if (tools_chkMobileOS() == "ios") {
                                i_anti_move_abnormality = parseInt(time * 100);
                                if (id_anti_move_abnormality != null) {
                                        clearTimeout(id_anti_move_abnormality);
                                        id_anti_move_abnormality = null;
                                }
                                id_anti_move_abnormality = setTimeout(function () { i_anti_move_abnormality = -1; }, 3000);
                        }
                ...
                setVolume: function (vol) {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.setVolume(" + vol + ") try"); }

                        // 조건검사
                        if (b_loaded_media == false) { return; }

                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.setVolume(" + vol + ")"); }
                ...
                setLoop: function (p_loop) {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.setLoop(" + p_loop + ") try"); }

                        // 조건검사
                        if (b_loaded_media == false) { return; }

                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.setLoop(" + p_loop + ")"); }
                ...
                setPlaybackRate: function (rate) {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.setPlaybackRate(" + rate + ") try"); }

                        // 조건검사
                        if (b_loaded_media == false) { return; }

                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.setPlaybackRate(" + rate + ")"); }
                ...
                setStopReserve: function (end) {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.setStopReserve(" + end + ")"); }
                ...
                castChangeMedia: function (p_media, p_data, p_subtitle) {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.castChangeMedia(" + p_media + ", " + p_data + ", " + p_subtitle + ")"); }
                        ...
                        // 안드로이드 특수 예외 처리
                        if (tools_chkMobileOS() == "android") {
                                // 입력 방어
                                b_disable_input = true;

                                // 제거 예약
                                setTimeout(function () { _this.clearMediaElement(); }, 250);
                                setTimeout(function () {
                                        // 재정의
                ...
                createMediaElement: function (p_media, p_dev) { // * 주의 : 로드가 끝나야 유효한 duration 을 받을 수 있고, 정보 정리가 가능함
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.createMediaElement(" + p_media + ", " + p_dev + ")"); }
                        ...
                        switch (_parent._category()) {
                                case "audio":
                                        ...
                                        if (_parent._option().preload == true) { obj.preload = "auto"; }
                                        break;
                                case "video":
                        ...
                        // ios 프리로드
                        if (tools_chkMobileOS() == "ios" && _parent._option().preload == true) {
                                _this.setCoreState(-1);
                                obj.load();
                        }
                ...
                clearMediaElement: function () {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.clearMediaElement()"); }
                        ...
                        // 제거
                        $(_parent._media()).attr("src", "");
                        $(_parent._media()).empty().remove();
                },

                // 코어 상태 설정
                setCoreState: function (st) {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.setCoreState(" + st + ")"); }
                ...
                updateUI: function () {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.updateUI()"); }
                ...
                onInteraction: function () {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.onInteraction()"); }
                        ...
                        if (_parent._module().follow != null) { _parent._module().follow.onInteraction(); }
                },

                // 이벤트 연결, 해제
                addEL: function () {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.addEL()"); }
                ...
                removeEL: function () {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.removeEL()"); }
                ...
                getState: function () {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.getState()"); }
                ...
                setState: function (obj) {
                        // 개발
                        if (dev_trace_detail == _parent._id()) { console.log("# core.setState(" + obj + ")"); }
